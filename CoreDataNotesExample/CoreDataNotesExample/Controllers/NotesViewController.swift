...
class NotesViewController: UIViewController {
    ...
    @IBAction func addNoteButtonAction(_ sender: UIBarButtonItem) {
        ...
        addNoteVC.saveNote = { [weak self] noteText, priorityColor in
            ...
            newNote.setValue(Date(), forKey: #keyPath(Note.date))
            ...
            AppDelegate.shared.coreDataStack.saveContext() // Save changes in CoreData
...
extension NotesViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let deleteAction = UIContextualAction(style: .destructive, title: nil) { _, _, complete in
            ...
            AppDelegate.shared.coreDataStack.managedContext.delete(self.notes[indexPath.row])
